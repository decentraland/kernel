...
export const onPositionSettledObservable =
  new Observable<InstancedSpawnPoint>();
...
export function loadParcelScene(
  ...
  if (!parcelSceneWorker) {
    parcelSceneWorker = new SceneSystemWorker(
      parcelScene,
      transport,
      persistent
    );
...
function reportPendingScenes() {
  ...
  for (const [sceneId, sceneWorker] of loadedSceneWorkers) {
    ...
    const failedLoading =
      (sceneWorker.ready & SceneWorkerReadyState.LOADING_FAILED) !== 0;
    if (shouldBeCounted) {
      countableScenes++;
    }
...
function setDesiredParcelScenes(desiredParcelScenes: Set<string>) {
  ...
  const newSet = (parcelSceneLoadingState.desiredParcelScenes =
    desiredParcelScenes);
...
async function loadParcelSceneByIdIfMissing(sceneId: string) {
  ...
  if (!getSceneWorkerBySceneID(sceneId)) {
    ...
    if (
      ...
      const scene = new UnityScene({
        ...
        data: ILandToLoadableParcelScene(
          parcelSceneLoadingState.isolatedModeOptions?.payload?.land
        ),
        mappings: [],
      });
      ...
      const parcelScene = new UnityParcelScene(
        ILandToLoadableParcelScene(parcelSceneToStart)
      );
  ...
  const observer = sceneLifeCycleObservable.add((sceneStatus) => {
    ...
    if (
      ...
      (worker.ready & SceneWorkerReadyState.STARTED) === 0
    ) {
...
export async function enableParcelSceneLoading() {
  ...
  lifecycleManager.on(
    "Scene.shouldPrefetch",
    async (opts: { sceneId: string }) => {
  ...
  lifecycleManager.on(
    "Position.settled",
    async (opts: { spawnPoint: InstancedSpawnPoint }) => {
...
export function allScenesEvent<T extends IEventNames>(
  data: AllScenesEvents<T>
) {
