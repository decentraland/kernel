...
export class ParcelLifeCycleController extends EventEmitter {
  ...
  updateLoadedParcels(
    secureRadius: number = 0
  ): ParcelSightSeeingReport | undefined {
    if (this.currentPosition === undefined) return undefined;

    this.isTargetPlaced = true;
    const sightedParcels = parcelsInScope(
      this.config.lineOfSightRadius,
      this.currentPosition
    );
    ...
    const secureParcels = new Set(
      parcelsInScope(
        this.config.lineOfSightRadius + secureRadius,
        this.currentPosition
      )
    );
    ...
    this.currentlySightedParcels = new Set(
      currentlyPlusNewlySightedParcels.filter(($) => secureParcels.has($))
    );
