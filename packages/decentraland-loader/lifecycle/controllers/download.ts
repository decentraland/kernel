...
export class SceneDataDownloadManager {
  ...
  async resolveSceneSceneIds(tiles: string[]): Promise<TileIdPair[]> {
    ...
    if (missingTiles.length > 0) {
      const scenesData = await fetch(
        `${this.options.catalystServer}/content/entities/scene?pointer=${missingTiles}`
      );

      const scenes = await scenesData.json();

      console.log(scenes, 1239);
      ...
      for (const scene of scenes) {
        ...
        const pendingSceneData =
          this.sceneIdToLandData.get(sceneId) || future<ILand | null>();
  ...
  async resolveSceneSceneId(pos: string): Promise<string | null> {
    return this.resolveSceneSceneIds([pos]).then((pairs) =>
      pairs.length > 0 ? pairs[0][1] : null
    );
  }
  ...
  setSceneRoots(contents: SceneMappingResponse) {
    for (const result of contents.data) {
      ...
      const promised =
        this.positionToSceneId.get(result.parcel_id) || future<string | null>();
