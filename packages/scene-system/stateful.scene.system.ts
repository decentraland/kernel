...
import type {
  ILogOpts,
  ScriptingTransport,
} from "decentraland-rpc/lib/common/json-rpc/types";
    ...
    this.eventSubscriber.on("stateEvent", ({ data }) => {
      ...
      if (type === "PublishSceneState") {
        this.sceneStateStorage
          ...
          .catch((error) =>
            this.error(`Failed to store the scene's state`, error)
          );
      }
    });
    ...
    this.eventSubscriber.on("stateEvent", ({ data }) => {
      if (data.type === "SaveSceneState") {
        this.sceneStateStorage
          ...
          .catch((error) =>
            this.error(`Failed to save the scene's manifest`, error)
          );
      }
    });
    ...
    this.eventSubscriber.on("stateEvent", ({ data }) => {
      ...
      if (type === "SaveProjectInfo") {
        this.sceneStateStorage
          ...
          .catch((error) =>
            this.error(`Failed to save the scene's info`, error)
          );
      }
    });
  }
