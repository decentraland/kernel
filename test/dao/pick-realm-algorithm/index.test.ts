...
describe("Pick realm algorithm default configuration", () => {
  ...
  it("Should select using large latency first", () => {
    const candidate = algorithm.pickCandidate(
      buildCandidates({ elapsed: 4000, catalystName: "second", usersCount: 1000 },
        ...
        { elapsed: 4500, catalystName: "third", usersCount: 10000 }), [0, 0])
  ...
  it("Should select using all peers third", () => {
    const candidate = algorithm.pickCandidate(
      buildCandidates({ catalystName: "second", usersCount: 5100, elapsed: 2000 },
        { catalystName: "first", usersCount: 5000 },
        { catalystName: "fourth", usersCount: 2000 },
        { catalystName: "third", usersCount: 3000 }), [0, 0])
  ...
  it("Should select using load balancing last", () => {
    const candidate = algorithm.pickCandidate(
      buildCandidates({ catalystName: "second", usersCount: 2000, domain: "foo.bar" },
        { catalystName: "first", usersCount: 2000, domain: "asd.fgh" },
        { catalystName: "third", usersCount: 2000, domain: "foo.bar" }), [0, 0])
  ...
  it("should pick the first one if all the candidates are the same", () => {
    const candidate = algorithm.pickCandidate(
      buildCandidates({ catalystName: "first" },
        { catalystName: "second" },
        { catalystName: "third" }), [0, 0])
