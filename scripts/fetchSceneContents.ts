...
const contentServerUrl =
  process.env.CONTENT_SERVER_URL ?? "https://peer-lb.decentraland.org";
...
async function main() {
  const sceneDataUrl = sceneId
    ? `${contentServerUrl}/content/entities/scene?id=${sceneId}`
    : `${contentServerUrl}/content/entities/scene?pointer=${parcel}`;

  const fetchSceneData = await fetch(sceneDataUrl);

  const sceneData = sceneId
    ? await fetchSceneData.json()
    : (await fetchSceneData.json())[0];
      ...
      .then(async (response) => {
        if (response.ok) {
          ...
          if (pathParts.length > 0) {
            ...
            await fs.promises.mkdir(`${scenePath}/${folder}`, {
              recursive: true,
            });
          }
          const fileStream = fs.createWriteStream(
            `${scenePath}/${content.file}`
          );
